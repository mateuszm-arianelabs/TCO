...
async function performTokenApproval(bnbUsdPrice: number, tokenAddress: Address): Promise<CostEstimate> {
...
async function addLiquidity(gasPrice: bigint, bnbUsdPrice: number): Promise<CostEstimate> {
  const routerContractPath = path.join(ARTIFACTS_PATH, "PancakeRouter.sol/PancakeRouter.json")
  const routerContractArtifact = await fs.readFile(routerContractPath, "utf-8")
  const routerContract = JSON.parse(routerContractArtifact);
  const routerContractAbi = routerContract["abi"];
  ...
  logCostEstimate("Add liquidity", costEstimate);
...
async function estimateSwapContractsCosts() {
  ...
  try {
    ...
    const factoryDeployment = await benchmark(() => estimateFactoryDeployment(gasPrice, bnbUsdPrice));
    const createPair = await benchmark(() => estimateCreatePair(gasPrice, bnbUsdPrice));
    const routerDeployment = await benchmark(() => estimateRouterDeployment(gasPrice, bnbUsdPrice));
    const tokenApproval = await benchmark(() => performTokenApproval(bnbUsdPrice, TOKEN_1_ADDRESS)); // this action cannot be estimated and it's mainnet cost must be paid
    const tokenApproval2 = await benchmark(() => performTokenApproval(bnbUsdPrice, TOKEN_2_ADDRESS)); // this action cannot be estimated and it's mainnet cost must be paid
    const estimateAddLiquidity = await benchmark(() => addLiquidity(gasPrice, bnbUsdPrice));
    ...
    const totalCostInWei = factoryDeployment.costInWei +
      ...
      tokenApproval2.costInWei +
        estimateAddLiquidity.costInWei;
    ...
    console.log(`Token Approval2: ${tokenApproval2.costInBNB} BNB ($${tokenApproval2.costInUSD})`);
    console.log(`Add liquidity: ${estimateAddLiquidity.costInBNB} BNB ($${estimateAddLiquidity.costInUSD})`);
